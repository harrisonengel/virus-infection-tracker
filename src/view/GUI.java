...
public class GUI {
	...
	private String firstName, middleName, lastName, ssn, sex, city, state, dateInfected;
	private int age;
	/**
	...
	private void initialize() {
		...
		lblTitle.setIcon(new ImageIcon(GUI.class.getResource("/view/virus 2.jpg")));
		panel_header.add(lblTitle);
		
		JLabel lblNewLabel = new JLabel("New label");
		lblNewLabel.setIcon(new ImageIcon(GUI.class.getResource("/view/virus3.jpg")));
		lblNewLabel.setBounds(10, 0, 205, 156);
		panel_header.add(lblNewLabel);
		
		JLabel lblNewLabel_1 = new JLabel("New label");
		lblNewLabel_1.setIcon(new ImageIcon(GUI.class.getResource("/view/virus3.jpg")));
		...
		btnAdd.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					JOptionPane. showMessageDialog(frame, "Error reading from file. Make sure it exists and is not corrupted!");
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (returnVal == JFileChooser.APPROVE_OPTION) {
					...
					try (FileWriter fw = new FileWriter(fc.getSelectedFile())) {
						...
						JOptionPane. showMessageDialog(frame, "Error saving file. Make sure you are using a .txt extension!");
					}
				}
			}
		});
	 ...
	 */
	private void patientPrompt(){
		patientPanel prompt = new patientPanel();
		
		try{
			JOptionPane.showConfirmDialog(null, prompt);
			firstName = prompt.getFirst();
			lastName = prompt.getLast();
			middleName = prompt.getMiddle();
			ssn = prompt.getSsn();
			age = prompt.getAge();
			sex = prompt.getGender();
			city = prompt.getCity();
			state = prompt.getState();
			dateInfected = prompt.getDate();
		} catch (NumberFormatException nFE){
			//Display error message saying something went terribly wrong!!
			JOptionPane. showMessageDialog(frame, "Error, incorrect input. Make sure your age is an integer number, not a spelled out number (like \"two\").");
		}
	...
	//Uses a JFileCooser to get a path, which can then be passed into an 
		//EasyReader object.
		public String getFileName(JFrame frame) {
			JFileChooser fc = new JFileChooser();
